...
camnasjl.so: build/build.jl src/CAMNAS.jl src/mna_solver.jl src/config.jl build/precompile_statements.jl
	JULIA_CPU_TARGET="native" $(JULIA) --project=. --threads=8 --startup-file=no build/build.jl $(TARGET)

wrapper.o:
	gcc -c -fPIC -O2 -I$(ROOT_DIR)/CAMNASCompiled/include -I/home/bauer/Codebase/dpsim/dpsim/include -I$(ROOT_DIR)/../../../include build/dpsim_wrapper.c
